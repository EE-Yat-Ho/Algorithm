...
let V = input[0] // 점 갯수
let E = input[1] // 간선 갯수
let K = Int(readLine()!)! // 시작 점

var lineArr = Array(repeating: Array(repeating: (0, 0), count: 0), count: V + 1)
var answerArr = Array(repeating: Int.max, count: V + 1)
...
var minHeap = MinHeap<IdxAndWeight>(IdxAndWeight(idx: K, weight: 0))
answerArr[K] = 0
...
while !minHeap.isEmpty {
    ...
    // 최소값이 아니면 스킵
    if answerArr[idx] < weight { continue }
    
    lineArr[idx].forEach {
        
        let startIdx = idx
        let endIdx = $0.0
        let weight = answerArr[startIdx] + $0.1
        
        if answerArr[endIdx] > weight {
            answerArr[endIdx] = weight
